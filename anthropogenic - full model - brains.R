...
if(sum(is.na(x$BrainSize)>0)){x<-x[-which(is.na(x$BrainSize)),]}
...
gelmanprior<-list(B=list(mu=rep(0,25), #mu has to be the number of parameters to be estimated (k-1 for each category of size k, 1 for each continuous variable, 1 for the intercept) 
                         V=gelman.prior(~zBrain+zWoS+zMat+zMass+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
...
mod<-MCMCglmm(as.factor(Anthropogenic)~zBrain+zWoS+zMat+zMass+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
...
#if you're only running this model for fun, I strongly suggest stopping here
...
save(Final.disp,file="material-full-brain.Rdata")
...
for(i in 1:100){ #loop through 100 trees
  ...
  mod<-MCMCglmm(as.factor(Anthropogenic)~zBrain+zWoS+zMat+zMass+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
                ...
                start= start1.l,
                data = x,
                nitt=30000, #this combo of nitt/thin/burnin will result in 10 samples / tree -- you might want to change that
                thin=2000,
                burnin=10000,
                pl=TRUE,
                pr=TRUE,
  ...
  print(i) #print which tree you're on (as the loop runs)
  ...
  save(Final.disp,file="material-full-brain.Rdata") #save, to make yourself feel better; if you omit this step, it will still save at the end, and will run slightly faster
