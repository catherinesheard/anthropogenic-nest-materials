...
mod<-MCMCglmm(as.factor(Plastic)~zWoS+zMat+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
...
save(Final.disp,file="plastic-nomass.Rdata")
...
setwd("C:/Users/cathe/Dropbox/LOCKDOWN WOOT/Bristol/Nest materials/")
setwd("C:/Users/cs16502/Dropbox/LOCKDOWN WOOT/Bristol/Nest materials/")
x<-read.csv("NestMaterialsData-WoS.csv")
x$PhyloName<-gsub(" ", "_",x$PhyloName)
x<-x[-which(x$duplicate==1),]
library(MCMCglmm)

trees<-read.tree("tree0099full.tre") #there are 10,000 topologies in the posterior distribution. this is a random hundred of them.
t100<-trees[1:100] #let's just take the first 100 for now
...
mod<-MCMCglmm(as.factor(Plastic)~zWoS+zMat+zMass+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
              +as.factor(loc.artificial)+as.factor(loc.ground.water)+as.factor(loc.cavity)+as.factor(loc.rock)+as.factor(loc.veg),
              random=~PhyloName, 
              ginverse=list(PhyloName=animalA), 
...
save(Final.disp,file="plastic-noRS.Rdata")
...
mod<-MCMCglmm(as.factor(Plastic)~zWoS+zMat+zMass+zRange+zHFI+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
              +as.factor(loc.artificial)+as.factor(loc.ground.water)+as.factor(loc.cavity)+as.factor(loc.rock)+as.factor(loc.veg),
              random=~PhyloName, 
              ginverse=list(PhyloName=animalA), 
...
save(Final.disp,file="plastic-noIUCN.Rdata")
