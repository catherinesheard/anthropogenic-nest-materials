...
mod<-MCMCglmm(as.factor(Plastic)~zBrain+zWoS+zMat+zMass+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
...
save(Final.disp,file="plastic-full-brain.Rdata")
