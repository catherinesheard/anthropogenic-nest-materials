...
if(sum(is.na(x$Anthropogenic)>0)){x<-x[-which(is.na(x$Anthropogenic)),]}
...
gelmanprior<-list(B=list(mu=rep(0,25), #mu has to be the number of parameters to be estimated (k-1 for each category of size k, 1 for each continuous variable, 1 for the intercept) 
                         V=gelman.prior(~zWoS+zMat+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
...
mod<-MCMCglmm(as.factor(Anthropogenic)~zWoS+zMat+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
...
save(Final.disp,file="material-noMass.Rdata")
...
#set up priors for this model
gelmanprior<-list(B=list(mu=rep(0,25), #mu has to be the number of parameters to be estimated (k-1 for each category of size k, 1 for each continuous variable, 1 for the intercept) 
                         V=gelman.prior(~zWoS+zMat+zMass+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
                                        +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
                                        +as.factor(loc.artificial)+as.factor(loc.ground.water)+as.factor(loc.cavity)+as.factor(loc.rock)+as.factor(loc.veg), #you'll have to adjust this equation to match your linear model, and mu above may have to change
                                        data = x,  scale=1+pi^2/3)), 
                  R=list(V=1,fix=1),G=list(G1=list(V=1E-10,nu=-1)))
...
mod<-MCMCglmm(as.factor(Anthropogenic)~zWoS+zMat+zMass+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
              +as.factor(loc.artificial)+as.factor(loc.ground.water)+as.factor(loc.cavity)+as.factor(loc.rock)+as.factor(loc.veg),
              random=~PhyloName, 
              ginverse=list(PhyloName=animalA), 
...
save(Final.disp,file="material-noRS.Rdata")
...
trees<-read.tree("100birdtrees.tre") #100 trees from the Jetz et al. 2012 Global Bird Tree
t100<-trees[1:100] #this step isn't necessary if your code runs perfectly, but it's nice to save a copy and then trim is rather than have to re-load the tree if something goes wrong
<-trees[1:100] #let's just take the first 100 for now
...
#set up priors for this model
gelmanprior<-list(B=list(mu=rep(0,23), #mu has to be the number of parameters to be estimated (k-1 for each category of size k, 1 for each continuous variable, 1 for the intercept) 
                         V=gelman.prior(~zWoS+zMat+zMass+zRange+zHFI+as.factor(Biome.simplified)
                                        +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
                                        +as.factor(loc.artificial)+as.factor(loc.ground.water)+as.factor(loc.cavity)+as.factor(loc.rock)+as.factor(loc.veg), #you'll have to adjust this equation to match your linear model, and mu above may have to change
                                        data = x,  scale=1+pi^2/3)), 
                  R=list(V=1,fix=1),G=list(G1=list(V=1E-10,nu=-1)))
...
mod<-MCMCglmm(as.factor(Anthropogenic)~zWoS+zMat+zMass+zRange+zHFI+as.factor(Biome.simplified)
              +as.factor(str.scrapenone)+as.factor(str.cup)+as.factor(str.platform)+as.factor(str.dome)+as.factor(str.ex)
              +as.factor(loc.artificial)+as.factor(loc.ground.water)+as.factor(loc.cavity)+as.factor(loc.rock)+as.factor(loc.veg),
              random=~PhyloName, 
              ginverse=list(PhyloName=animalA), 
...
save(Final.disp,file="material-noiucn.Rdata")
...
#set up priors for this model #23
gelmanprior<-list(B=list(mu=rep(0,21), #mu has to be the number of parameters to be estimated (k-1 for each category of size k, 1 for each continuous variable, 1 for the intercept) 
...
mod<-MCMCglmm(as.factor(Anthropogenic)~zWoS+zMat+zMass+zRange+zHFI+as.factor(IUCN.simplified)+as.factor(Biome.simplified)
              +as.factor(loc.artificial)+as.factor(loc.ground.water)+as.factor(loc.cavity)+as.factor(loc.rock)+as.factor(loc.veg),
              random=~PhyloName, 
              ginverse=list(PhyloName=animalA), 
              prior = gelmanprior, 
...
save(Final.disp,file="material-noStr.Rdata")
